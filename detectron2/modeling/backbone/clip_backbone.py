...
class ModifiedResNet(Backbone):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64, 
        ...
        ##################### LoRA layers
        self.dia_1 = self._make_ada(64, 32, 256)
        self.dia_2 = self._make_ada(256, 128, 512, True)
        self.dia_3 = self._make_ada(512, 256, 1024, True)
        if 'res5' in out_features:  # FPN
            self.dia_4 = self._make_ada(1024, 512, 2048, True)
        else:  # C4, layer4 created here won't be used in backbone, but used in roi_head
            self.dia_4 = self._make_ada(1024, 512, 2048, True)
        self.dsa_1 = self._make_ada(256, 32, 256) 
        self.dsa_2 = self._make_ada(512, 128, 512) 
        self.dsa_3 = self._make_ada(1024, 256, 1024) 
    ...
    ################
    def _make_ada(self, inplanes, midplanes, outplanes, downsample=False):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layer1)):
            ...
            if i == 0:
                temp = x
        x = x + x_ada #self.relu(x + x_ada)
        dis_feat['res2'] = x
        diff = self.dsa_1(temp - x)
        dsa_feat['res2'] = x * diff
        x = x + x * diff
        dia_feat['res2'] = dis_feat['res2'] #x_ada
        ...
        x = x + x_ada #self.relu(x + x_ada)
        dis_feat['res3'] = x
        diff = self.dsa_2(temp - x)
        dsa_feat['res3'] = x * diff
        x = x + x * diff
        dia_feat['res3'] = dis_feat['res3'] #x_ada
        ...
        x = x + x_ada #self.relu(x + x_ada)
        dis_feat['res4'] = x
        diff = self.dsa_3(temp - x)
        dsa_feat['res4'] = x * diff
        x = x + x * diff
        dia_feat['res4'] = dis_feat['res4'] #x_ada
        ...
        if self.pool_vec:  # pool a vector representation for an image, for global image classification
            ...
            return outputs, dia_feat, dsa_feat, dis_feat
