...
class PromptLearner(nn.Module):
    ...
    def forward(self, x_di, x_ds):
        ...
        # x_di, x_ds: [[1024, 256, 14, 14] [1024, 512, 14, 14] [1024, 1024, 14, 14]]

        #ctx_di = self.di_layer(x_di.half()).reshape(self.n_ctx_di, -1)        # [8, 512]
        ctx_di = x_di.half().reshape(self.n_ctx_di, -1)        # [8, 512]
        ctx_dim = ctx_di.size(-1)
        #ctx_ds = self.ds_layer(x_ds.half()).reshape(self.n_ctx_ds, -1)        # [8, 512]
